      ...
      def json_list
        all_from_json.map do |rental|
...

          {
            id: rental.id,
            actions: [
              Action.to_json('driver', 'debit', price.total + rental.deductible_amount),
              Action.to_json('owner', 'credit', price.total - fee.total),
              Action.to_json('insurance', 'credit', fee.insurance),
              Action.to_json('assistance', 'credit', fee.assistance),
              Action.to_json('drivy', 'credit', fee.drivy + rental.deductible_amount)
            ]
          }
        end
      end
